...
import NextSlide from "../assets/nextButton.svg"
 ...
 let [current, setCurrent] = useState(0);
  ...
  return (
    ...
    {slides.map((slide, index) =>(
        ...
        >
        <img className="min-h-[400px] object-cover"
        src={slide.src} 
        alt={`Slideshow image ${index}`} 
        />
        <div className="absolute left-0 top-44 flex h-96 w-full flex-col gap-2
        bg-gray-800/[0.6] p-4 text-white lg:gap-4">
        <p>{slide.text}</p>
    ...
    <img onClick={previousSlide}  className="absolute bottom-5 right-16
     h-7 w-7 rounded-full border p-1 hover:cursor-pointer"
    src={Previous}
    alt="Previous slide button" 
    />
    <img onClick={nextSlide}  className="absolute bottom-5 right-5
     h-7 w-7 rounded-full border p-1 hover:cursor-pointer"
    src={NextSlide}
